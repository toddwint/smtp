...
if __name__ == '__main__':
    ...
    menu_log_fzf = [
        (f"{appname} log (/var/mail/root)", f"{tailcmd} {logsdir}/{appname}.log | {fzfcmd}"),
        ]
    menu_log = [
        (f"{appname} log (/var/mail/root)", f"more {logsdir}/{appname}.log"),
        ]
    menu_configuration = [
        (f"{appname} configuration", "postconf -x | more"),
        (f"{appname} domain configuration", "postconf -x mydomain | more"),
        (f"{appname} hostname configuration", "postconf -x myhostname | more"),
        (f"{appname} destination configuration", "postconf -x mydestination | more"),
        (f"{appname} networks configuration", "postconf -x mynetworks | more"),
        (f"{appname} alias map configuration", "more /etc/postfix/virtual"),
    ...
    menu_debug = [
        ("Send test email", f"{debugdir}/send_test_mail.sh"),
